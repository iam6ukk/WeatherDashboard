...
class DateTimeUtil {
    ...
    static trimEmptyPlaceholders(value, prompt) {
        const result = value.replace(new RegExp(this.escapeRegExp(prompt !== null && prompt !== void 0 ? prompt : '_'), 'g'), '');
        return result;
    }
    ...
    static toTwelveHourFormat(value) {
        let hour = parseInt(value.replace(new RegExp(this.escapeRegExp(this._parser.prompt), 'g'), '0'), 10);
        if (hour > 12) {
            hour -= 12;
        }
        else if (hour === 0) {
