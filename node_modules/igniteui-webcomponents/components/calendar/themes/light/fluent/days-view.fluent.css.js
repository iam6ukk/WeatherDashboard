...
export const styles = css `:host {
  --date-size: max(var(--is-large, 1) * max(1rem, -1 * 1rem), var(--is-medium, 1) * max(0.875rem, -1 * 0.875rem), var(--is-small, 1) * max(0.75rem, -1 * 0.75rem));
}
...
[part~=current][part~=single][part~=date-inner]::after {
  border-radius: clamp(0rem, calc(var(--ig-radius-factor, 1) * 1rem), 1rem);
  content: "";
  position: absolute;
  top: 0.0625rem;
